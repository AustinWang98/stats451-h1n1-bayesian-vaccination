...
$$
y_i \sim \mathrm{Bernoulli}\!\left(\mathrm{logit}^{-1}\!\left(\alpha_{g_i} + x_i^\top \beta\right)\right)
$$

where $g_i \in \{1,\ldots,J\}$ is the group index for person $i$—either **education level** (`qualification`) or **MSA category** (`census_msa`), depending on the model.
