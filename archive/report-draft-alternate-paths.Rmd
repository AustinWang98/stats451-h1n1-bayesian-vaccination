---
title: "Data Science 451 Final Project Report"
output: pdf_document
date: "2024-12-10"
---
...
knitr::include_graphics("/Users/eriksen/Desktop/451 Final Project/eda1.png")
```
...
```{r, echo=FALSE, out.width="70%",fig.align='center'}
knitr::include_graphics("/Users/eriksen/Desktop/451 Final Project/eda2.png")
```
...
```{r, echo=FALSE, out.width="70%",fig.align='center'}
knitr::include_graphics("/Users/eriksen/Desktop/451 Final Project/grouped_analysis.png")
```


\section{Results}
After performing stan model compilation and Markov Chain Monte Carlo Sampling (2000 iterations for each model) for both of our models, here are the results of the posterior distributions of key parameters from the hierarchical Bayesian model with \texttt{census\_msa}, which can show us the effects of individual predictors ($\beta$), group-level intercepts ($\alpha$), and hyperparameters ($\mu_\alpha$ and $\sigma_\alpha$) on vaccination behavior.

```{r, echo=FALSE, out.width="60%",fig.align='center'}
knitr::include_graphics("/Users/eriksen/Desktop/451 Final Project/result3.png")
```


The $\beta$ coefficients emphasize the importance of individual predictors. Factors such as \texttt{dr\_recc\_h1n1\_vacc} (H1N1 flu vaccine was recommended by doctor, which is beta[7] that has a value of 2.05) and \texttt{h1n1\_worry} (Is a healthcare worker, which is beta[11] and has a value of 0.83) show posterior distributions concentrated away from zero, indicating significant positive effects on vaccination likelihood. In contrast, predictors with distributions centered closer to zero, such as beta[15] that has a value of just -0.01, are likely to have less influence. In order for better visualization, we added the names of predictors for each beta, ensuring a better understanding of each predictor's influence.

```{r, echo=FALSE, out.width="70%",fig.align='center'}
knitr::include_graphics("/Users/eriksen/Desktop/451 Final Project/result1.png")
```

\[
\begin{array}{|c|c|c|c|}
\hline
\textbf{Beta} & \textbf{Predictor} & \textbf{Beta} & \textbf{Predictor} \\ \hline
\text{beta[1]} & \text{h1n1\_worry} & \text{beta[14]} & \text{is\_h1n1\_risky} \\ \hline
\text{beta[2]} & \text{h1n1\_awareness} & \text{beta[15]} & \text{sick\_from\_h1n1\_vacc} \\ \hline
\text{beta[3]} & \text{antiviral\_medication} & \text{beta[16]} & \text{is\_seas\_vacc\_effective} \\ \hline
\text{beta[4]} & \text{contact\_avoidance} & \text{beta[17]} & \text{is\_seas\_risky} \\ \hline
\text{beta[5]} & \text{bought\_face\_mask} & \text{beta[18]} & \text{sick\_from\_seas\_vacc} \\ \hline
\text{beta[6]} & \text{avoid\_large\_gatherings} & \text{beta[19]} & \text{age\_bracket} \\ \hline
\text{beta[7]} & \text{dr\_recc\_h1n1\_vacc} & \text{beta[20]} & \text{qualification} \\ \hline
\text{beta[8]} & \text{dr\_recc\_seasonal\_vacc} & \text{beta[21]} & \text{race} \\ \hline
\text{beta[9]} & \text{chronic\_medic\_condition} & \text{beta[22]} & \text{sex} \\ \hline
\text{beta[10]} & \text{cont\_child\_under\_6\_mnths} & \text{beta[23]} & \text{marital\_status} \\ \hline
\text{beta[11]} & \text{is\_health\_worker} & \text{beta[24]} & \text{housing\_status} \\ \hline
\text{beta[12]} & \text{has\_health\_insur} & \text{beta[25]} & \text{employment} \\ \hline
\text{beta[13]} & \text{is\_h1n1\_vacc\_effective} & \text{beta[26]} & \text{no\_of\_children} \\ \hline
\end{array}
\]

For the $\alpha$ parameters, which are group-level intercepts for \texttt{census\_msa}, the posterior distributions appear to be similar across different groups, which the results to be -6.97, -7.02, and -6.97 for each level of region, respectively. However, the hyperparameter $\sigma_\alpha$, which shows the variability of the $\alpha$ parameters, is large with a value of 7.01. This tells us that there is still meaningful variability across different locations, and vaccination rates that are not just explained by the individual predictors.

The hyperparameters $\mu_\alpha$ and $\sigma_\alpha$ summarize these group-level trends. The posterior for $\mu_\alpha$ is centered near zero, which aligns with our prior expectation. Meanwhile, the large $\sigma_\alpha$ highlights the importance of modeling \texttt{census\_msa} as a hierarchical effect, as it can get large differences between groups that may be overlooked if we use a simpler model.

```{r, echo=FALSE, out.width="70%",fig.align='center'}
knitr::include_graphics("/Users/eriksen/Desktop/451 Final Project/alpha1.png")
```

Overall, the results suggest that while some individual predictors are critical in explaining vaccination behavior, it is also important to add a hierarchical structure in our model in consideration of significant regional differences. The large variability presented by $\sigma_\alpha$ emphasize the complexity of geographic influences on vaccination rates. Similar strategies should also be used in other group-level variables for further study, and we hope this project can serve as a reference to those who want to make more efforts to increase vaccination rates and ensure the safety of more people on our planet.
...
Yiou Wang: 1. Contributed to half of the project proposal by writing the potential statistical models, prior and posteriors specification, and so on.  2. Performed data preprocessing and cleaning and EDA; Designed and implemented hierarchical Bayesian model where qualification is used as group-level intercept; and used LOO package for validation in R. 3. Created corresponding slides of the presentation. 4. Wrote "Why Hierarchical Bayesian Model Work", "Feature Selection", and "RStan Hierarchical Bayesian Model Explanation" sections in this paper.

Eriksen Cao: 1. Contributed to half of the project proposal by writing problems of interests related questions, statistical approaches, time line, etc. 2. Worked with Yiou to determine our strategies, helped with data cleaning, visualization, and both model implementations. Worked on the code implementation of grouped prediction analysis and posterior distribution. 3. Worked on corresponding slides of the presentation, and performed the presenation for our group. 4. Wrote "RStan Hierarchical Bayesian Model Explanation for Census MSA", "Using the LOO Package for Model Comparison", "Grouped Prediction Analysis", "Results" sections in this paper.

Zhengxue Zhu: Contributed to the project by writing "Introduction" and "Data Preprocessing and EDA" sections in this paper; provided clear explanations for the figures used in the report.
